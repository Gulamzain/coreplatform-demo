...
export default function ClosedTicketsPage() {
  ...
  const filteredTickets = closedTickets.filter(t => t.subject.toLowerCase().includes(searchTerm.toLowerCase()) || t.id.toLowerCase().includes(searchTerm.toLowerCase()));

  return (
    <div className="closed-tickets-page">
      <div className="page-header"><h1>Closed Tickets</h1><p>View your resolved support tickets</p></div>
      <div className="search-bar"><BiSearch size={18} /><input type="text" placeholder="Search tickets..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /></div>
      <div className="tickets-list">{filteredTickets.map((ticket, index) => (<motion.div key={ticket.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="ticket-card"><div className="ticket-header"><span className="ticket-id">{ticket.id}</span><span className="status closed">Closed</span></div><h3>{ticket.subject}</h3><div className="ticket-meta"><span><BiTime size={14} /> Created: {ticket.date}</span><span><BiCheckCircle size={14} /> Resolved: {ticket.resolvedDate}</span><span><BiMessage size={14} /> 4 messages</span></div><div className="ticket-rating"><span>Rating:</span>{[...Array(5)].map((_, i) => (<BiStar key={i} className={i < ticket.rating ? 'filled' : ''} size={14} />))}</div><button className="view-btn">View Details →</button></motion.div>))}</div>
